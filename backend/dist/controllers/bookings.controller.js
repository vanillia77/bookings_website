...
const createBooking = (req, res) => {
    const { userId, date, endDate, details, status, phone, persons } = req.body;
    if (!date) {
        return res.status(400).json({ message: "Date requise" });
    }
    const uId = req.user ? req.user.id : (userId || 1);
    const initialStatus = status || 'pending';
    db_1.default.run("INSERT INTO bookings (userId, date, endDate, details, status, phone, persons) VALUES (?, ?, ?, ?, ?, ?, ?)", [uId, date, endDate, details || 'Reservation', initialStatus, phone, persons], function (err) {
        if (err) {
...
const updateBooking = (req, res) => {
    const { id } = req.params;
    const { date, endDate, details, status, phone, persons } = req.body;
    db_1.default.run("UPDATE bookings SET date = COALESCE(?, date), endDate = COALESCE(?, endDate), details = COALESCE(?, details), status = COALESCE(?, status), phone = COALESCE(?, phone), persons = COALESCE(?, persons) WHERE id = ?", [date, endDate, details, status, phone, persons, id], function (err) {
        if (err) {
